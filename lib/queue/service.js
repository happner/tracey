...
Service.prototype.listen = function(callback){
  ...
  _this.__queue.tpop(function(err, message, commit, rollback) {
    ...
    if (err){
      _this.services.log.error('issue with queue attach method' + err.toString());
      _this.state = _this.STATES.error;
    }

    _this.services.log.info('popped a message off the queue', err);
    ...
    /*
     ...
     test_script: 'test-benchmarket' #configured test in npm
     node_js:
     - '7'
     - '0.10'
     */
    ...
    _this.config.repos.every(function(repo){
      ...
      if (repoFound) _this.currentJob.message.config = repo;
...
Service.prototype.push = function(message, callback){
  ...
  try{
    ...
    _this.services.log.info('pushing message into queue: ' + jSON.stringify(message, null, 2));
