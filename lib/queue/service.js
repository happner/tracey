...
Service.prototype.listen = function(callback){
  ...
  queue.tpop(function(err, message, commit, rollback) {
    ...
    if (err){
      _this.services.log.error('issue with queue attach method', e);
    ...
    this.emit('message-popped', _this.currentOperation);
  });
...
Service.prototype.start = function(callback){
  ...
  try{
    ...
    var queue = new Queue({
      ...
      _this.state = _this.STATES.running;
...
Service.prototype.stop = function(callback){
  ...
  try{
    ...
    if (this.currentOperation) return this.currentOperation.rollback(callback, true)//noListen
